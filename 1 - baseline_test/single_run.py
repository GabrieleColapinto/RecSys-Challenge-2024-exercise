...
with open(parameters_path, "r") as parameters_file:
    ...
    write_json_file(hparams_to_dict(hparams), deliverables_folder / "NRMSDocVec_hparams.json")
    write_json_file(parameters, deliverables_folder / "NRMSDocVec_run_parameters.json")
    ...
    model = NRMSDocVec(hparams=hparams, seed=42)
    model.model.compile(
    ...
    eval_dataloader = NRMSDataLoader(
        behaviors=df_eval,
        article_dict=article_mapping,
        unknown_representation="zeros",
        history_column=DEFAULT_HISTORY_ARTICLE_ID_COL,
    ...
    # Add scores to df
    df_pred = add_prediction_scores(df_eval, scores.tolist())
    ...
    write_submission_file(
        ...
        filename_zip=f"{NRMSDocVec.__name__}-{seed}-{datasplit}.zip",
    )
