...
def get_args():
    parser = argparse.ArgumentParser(
        description="Argument parser for the modified NRMSModel training"
    )
    ...
    # DIRECTORY TO STORE THE RUN INFORMATION AND RESULTS
    parser.add_argument(
        "--results_dir",
        type=str,
        help="Directory containing the parameters of the run and its results",
        required=True
    )

    # ARGUMENT ENABLING THE POSITIONAL ENCODING
    parser.add_argument("--use_pe", action="store_true",
                        help="Enable positional encoding in user encoder")

    parser.add_argument("--use_rln", action="store_true",
                        help="Enable residual + layer normalization")

    parser.add_argument("--use_td", action="store_true",
                        help="Enable time decay / recency bias")
