...
class NRMSDocVecAblation:
    ...
    def _build_userencoder(self, titleencoder):
        ...
        # TIME DECAY OVER THE HISTORY SEQUENCE
        if getattr(self.hparams, "use_td", False):
        ...
        # Add position embeddings to inject order information into attention.
        if getattr(self.hparams, "use_pe", False):
            H = self.hparams.history_size
            D = self.hparams.head_num * self.hparams.head_dim  # same dim as click_title_presents last dim

            pos_emb = tf.keras.layers.Embedding(
                input_dim=H,
                output_dim=D,
                name="pos_embedding_history",
            )
            positions = tf.range(start=0, limit=H, delta=1)
            pe = pos_emb(positions)  # [H, D]
            pe = tf.expand_dims(pe, axis=0)  # [1, H, D] broadcast over batch

            click_title_presents = tf.keras.layers.Add()([click_title_presents, pe])
        ...
        if getattr(self.hparams, "use_rln", False):
            y = tf.keras.layers.Add()([click_title_presents, y])
