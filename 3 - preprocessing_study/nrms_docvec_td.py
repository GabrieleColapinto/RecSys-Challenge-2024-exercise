...
class NRMSDocVecTd:
    ...
    def _build_userencoder(self, titleencoder):
        ...
        # distance: [0,1,2,...,H-1] where 0 = oldest, H-1 = most recent
        H = self.hparams.history_size
        distances = tf.range(H, dtype=tf.float32)  # oldest->recent increasing
        # convert to "lag": most recent lag=0, oldest lag=H-1
        lags = tf.cast((H - 1), tf.float32) - distances

        alpha_init = float(getattr(self.hparams, "td_alpha_init", 0.05))
        alpha = tf.Variable(alpha_init, trainable=True, dtype=tf.float32, name="td_alpha")

        # weights: exp(-alpha * lag) -> recent (lag=0) weight=1
        w = tf.exp(-alpha * lags)  # [H]
        w = tf.reshape(w, (1, H, 1))  # broadcast to [B, H, 1]

        click_title_presents = tf.keras.layers.Multiply()([click_title_presents, w])
        ''' ===================== END OF MODIFICATIONS ====================================== '''

        # SELF ATTENTION OVER CLICKED SEQUENCE
    ...
    def _build_nrms(self):
        ...
        # ADDITIONAL INPUTS
        user_features = tf.keras.Input(shape=(self.hparams.user_feat_dim,), dtype="float32")
        his_item_features = tf.keras.Input(shape=(self.hparams.history_size, self.hparams.item_feat_dim),
                                           dtype="float32")
        pred_item_features = tf.keras.Input(shape=(None, self.hparams.item_feat_dim), dtype="float32")
        ...
        his_item_proj = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(OUTPUT_DIM, activation="relu")
        )(his_item_features)

        pred_item_proj = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(OUTPUT_DIM, activation="relu")
        )(pred_item_features)

        uf = tf.keras.layers.Dense(OUTPUT_DIM, activation="relu")(user_features)

        user_present = self.userencoder(his_input_title)     # docvec only
        user_present = tf.keras.layers.Add()([user_present, tf.reduce_mean(his_item_proj, axis=1)])
        user_present = tf.keras.layers.Add()([user_present, uf])

        news_present = tf.keras.layers.TimeDistributed(self.newsencoder)(pred_input_title)
        news_present = tf.keras.layers.Add()([news_present, pred_item_proj])

        preds = tf.keras.layers.Dot(axes=-1)([news_present, user_present])
        preds = tf.keras.layers.Activation(activation="softmax")(preds)

        # Model and scorer that include the new inputs
        model = tf.keras.Model(
            [his_input_title, pred_input_title, user_features, his_item_features, pred_item_features],
            preds
        )

        scores = tf.keras.layers.Dot(axes=-1)([news_present, user_present])  # (B, N)
