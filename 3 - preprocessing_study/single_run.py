...
with open(parameters_path, "r") as parameters_file:
    ...
    # ====================== PREPROCESSING ===================================
    use_premium_propensity = parameters["use_premium_propensity"]
    use_reading_engagement = parameters["use_reading_engagement"]
    use_article_length_propensity = parameters["use_article_length_propensity"]

    # =========== PREPROCESSING RELATED PARAMETERS OF THE MODEL ==============
    user_feat_dim = 0
    item_feat_dim = 0

    if use_premium_propensity:
        user_feat_dim += 1
        item_feat_dim += 1

    if use_reading_engagement:
        user_feat_dim += 1

    if use_article_length_propensity:
        user_feat_dim += 1
        item_feat_dim += 1
    ...
    # Preprocessing
    hparams.use_premium_propensity = use_premium_propensity
    hparams.use_reading_engagement = use_reading_engagement
    hparams.use_article_length_propensity = use_article_length_propensity
    ...
    # Article length integration
    if use_article_length_propensity:
        df_article_length = pl.read_parquet(NEW_DATA_PATH / "article_length.parquet")
        df_articles = df_articles.join(
            other=df_article_length,
            on=DEFAULT_ARTICLE_ID_COL,
            how="inner"
        )

        # I add the column to the list
        item_feature_cols.append("article_length")
    ...
    # Article length propensity data
    if use_article_length_propensity:
        # I retrieve the association between user and article length propensity
        df_article_length_propensity = pl.read_parquet(NEW_DATA_PATH / "article_length_propensity.parquet")

        # I add the article length propensity to the main dataframe
        df = df.join(
            other=df_article_length_propensity,
            on=DEFAULT_USER_COL,
            how="inner"
        )

        # I add the article length propensity to the evaluation dataframe
        df_eval = df_eval.join(
            other=df_article_length_propensity,
            on=DEFAULT_USER_COL,
            how="inner"
        ...
        # I append the column to the list
        user_feature_cols.append("article_length_propensity")
