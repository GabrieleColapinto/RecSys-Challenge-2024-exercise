...
def _pad_2d_int_lists_to_len(lists, pad_value: int, target_len: int) -> list[list[int]]:
    # normalize -> list[list[int]] of ints
    normalized = []
    for row in lists:
        if row is None:
            row = []
...
class NewNRMSDataLoaderPretransform(NewsrecDataLoader):
    ...
    def __post_init__(self):
        ...
        # map docvec on history_doc_idx / inview_doc_idx
        self.X = self.X.pipe(
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]:
        ...
        if self.eval_mode:
            ...
            # safe squeeze (if lookup_article_matrix has shape (n,1,D))
        ...
        if self.lookup_itemfeat_matrix is not None:
            ...
            # Use shapes which are coherent with docvec: eval -> (B,H,D) e (B,N,D)
            if self.eval_mode:
                B = len(batch_X)
